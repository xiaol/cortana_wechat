...
function getAnswer (chat_item,question) {
	...
	base.dataConn(requestUrl,requestData,"get",function(data){
		...
		answer = filterAnswer(data.answer)
		msg_send_item.item = chat_item;
...
function filterAnswer(answer){
	var asw = answer,noasw = "朕每天很忙的,这么简单的问题就不要来问了~";
	_console.log(answer)
	if(asw){
		if(asw.length>=3&&asw.substring(0,3)=="谢邀。"){
			asw = asw.substring(3,asw.length)
		}else if(asw.length>=2&&asw.substring(0,2)=="谢邀"){
			asw = asw.substring(2,asw.length)
		}
		asw = $.trim(asw);
		asw = (asw==""?noasw:asw);
	}else{
